...
const FirstFloo = () => {
  ...
  const fetchData = async () => {
    try {
      const response1 = await fetch("http://localhost:5000/api/getfifth");
      if (!response1.ok) {
        throw new Error("Failed to fetch data");
      }
      const data1 = await response1.json();
      
      const response2 = await fetch("http://localhost:5000/api/getsixth");
      if (!response2.ok) {
        throw new Error("Failed to fetch data");
      }
  ...
  const handleSave = async () => {
    try {
      ...
      const response1 = await AxiosAPI.put(`http://localhost:5000/api/thubes/${editingProduct._id}`, updatedProductData);
      const response = await AxiosAPI.put(`http://localhost:5000/api/thubess/${editingProduct._id}`, updatedProductData1);
